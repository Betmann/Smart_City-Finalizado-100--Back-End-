...
class TemperaturaFilterView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        valor_lte = request.data.get('valor_lte', None)
        timestamp_gte = request.data.get('timestamp_gte', None)
        timestamp_lte = request.data.get('timestamp_lte', None)
        ...
        if valor_lte:
            filters &= Q(valor__lte=valor_lte)
        if timestamp_gte:
            filters &= Q(timestamp__gte=timestamp_gte)
        if timestamp_lte:
            filters &= Q(timestamp__lte=timestamp_lte)
